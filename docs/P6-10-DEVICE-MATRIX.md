...
### 5.4 修后三点复验增补（2026-09-10，P6-11 延后设备任务；KernelSU × Android 16 同机）

> 完整执行记录 `docs/P6-10.md` §9；本小节为矩阵格内的摘要锚点。
> 本机格 runtime 状态升级：P6-10 时代 **pre-fix 1.32.0** → **post-fix 1.32.0**
> （版本恒等、代码面三原语命中；模块目录+当会话 bind 双轨落地，矩阵格持续有效）。

| 修复项 | 真机新证据（修后） | trace |
| :-- | :-- | :-- |
| D-P6-10-02 锁主围栏 | 删锁孪生接管：20s 单实例收敛；围栏行 `Lock-owner fence: live daemon (PID 20077) ... non-holder (PID 24552) exiting`；静置双窗审计**每 token op=tick=1**（pre-fix 病征 2–3 行/分） | `…171546-phaseD.log` / `…173500-phaseD-recheck.log` |
| D-P6-10-01 扫描年龄钳制 | 9 枚 25h 陈旧根（cycle 残留 + dag 清空）：**零重登记/零 pending/limit=0**；同窗新链 120s 登记→SUCCESS（不饿死） | 同上 |
| D-P6-10-03 终局拒重放 | attempt-2 落地即 kill（run#16 微窗口径，2s 粒度命中）：执行=2（≤1+max）、`retry.count=max`+`until` 清、终局后 +180s 零重放 | 同上 |

矩阵配套复跑：p6-device 时序加固版首跑 **56/0/0**（`…-162551-p6.log`）；
p1 11/0/2、p3 28/0 独立保持；`--with-device` 组合遭 18:0x 起用户 App 负载风暴
（loadavg 15–55）+尾段链路失联 → 环境性未竟，组合全绿待下一窗口（诚实登记，
同 §5.3.1/2 既往病征；宿主 51 套件段全绿）。

> **✅ 2026-09-11 00:05–01:08 补验窗（本机格）：`run_tests.sh --with-device` 组合单命令
> 全绿收口**——ALL SUITES GREEN / exit 0（trace `tests/results/run_tests-20260911-000539.log`：
> 宿主 51 套件 + 设备段 p1 **11/0/2** + p3 **28/0/0/BLOCKED=0** + p6 **56/0/0**@2497s）；
> 同夜分项单独复跑先行（p6 **56/0/0**@4565s=修后加固复合体第二次真机取证、p3 28/0、
> p1 11/0/2；serial `192.168.2.156:42037`）。起测 load ~10 平台期（未达 <6，靠挂起 adb
> 客户端定点解卡 ×3 保持预算内），WiFi adb 半死 tunnel 为风暴期工具侧病征（§5.3 家族
> 追加一条），产品判据零受损。动作序执行对照见 `docs/P6-10.md` §9.4/§9.5 补记。
...
- **KernelSU × Android 16 格**：P6-10 九必测场景全部真机达成（§2，run#18 56/0/0 +
  ...
  **组合 `run_tests.sh --with-device` 全绿**（2026-09-11 补验窗，§5 补记框收口证据）。
