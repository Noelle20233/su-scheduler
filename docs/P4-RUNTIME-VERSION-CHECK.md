...
- **Runtime 库版本 = 内部实现线**（D4 §5.2）：`system/bin/su-scheduler-runtime`
  头部 `RUNTIME_LIB_VERSION="1.19.0"`，随 § 功能集合递增，**不参与**模块 8 处一致
...
- **现状缺口（R4）**：`RUNTIME_LIB_VERSION` **未纳入任何构建期断言**。当前守护仅
  ...
  构建期发现，可能打包出「文档宣称 1.19.0、实际 1.18.0」的发布。
...
# 期望：输出含 "runtime lib version semantic: 1.19.0" 等 Runtime 断言，且全部 [PASS]
