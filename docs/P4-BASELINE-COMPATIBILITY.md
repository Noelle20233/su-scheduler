...
| B5 | Runtime 库 | `RUNTIME_LIB_VERSION`（现 1.22.0，P4-04 §20 WAITING 依赖门控递增）存在且语义化版本格式；模块发布时打包的库版本与文档记录一致 | `tests/p3-integration`（dash -n + selfcheck）+ `docs/P4-RUNTIME-VERSION-CHECK.md` 方案 | [PASS]；daemon log `Runtime library loaded (v1.22.0)` |
