...
| B5 | Runtime 库 | `RUNTIME_LIB_VERSION`（现 1.28.0，P4-09 可观测面 + P4-10 加固：GET_SUMMARY waiting 计数 / GET_TASK_DETAIL dependency/condition/dependency_state/gate_state / task status dependency=/Gate / dep 图切分改 `cut` mksh 兼容）存在且语义化版本格式；模块发布时打包的库版本与文档记录一致 | `tests/p3-integration`（dash -n + selfcheck）+ `docs/P4-RUNTIME-VERSION-CHECK.md` 方案 + `tests/p4-dependency` §hard-p4-10 版本断言 | [PASS]；daemon log `Runtime library loaded (v1.28.0)` |
...
| B8 | WebUI 数据面 | 只读 Reader 输出统一 JSON、JSON 转义防注入、三态、有界日志；WebUI 不直执 Root；**GET_SUMMARY/GET_TASK_DETAIL 只增键不删改**（P4-09：counts 增 `waiting`、task 增 dependency/condition/dependency_state/gate_state） | `tests/webui/read-only.test.sh`（P4-09 后 31）+ `tests/webui/security.test.sh`（15） | [PASS] |
