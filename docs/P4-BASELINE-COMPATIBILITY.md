...
| B5 | Runtime 库 | `RUNTIME_LIB_VERSION`（现 1.27.0，P4-09 可观测面：GET_SUMMARY waiting 计数 / GET_TASK_DETAIL dependency/condition/dependency_state/gate_state / task status dependency=/Gate）存在且语义化版本格式；模块发布时打包的库版本与文档记录一致 | `tests/p3-integration`（dash -n + selfcheck）+ `docs/P4-RUNTIME-VERSION-CHECK.md` 方案 | [PASS]；daemon log `Runtime library loaded (v1.27.0)` |
