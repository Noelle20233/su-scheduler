...
| **D-P5-05** | `tests/p3-device/smoke.sh` item 14（task start）+ 真机 daemon 窗口 | p3-device item 14 偶发 `task start edit1 → rc=5 operation_timeout, state=STARTING`（瞬时 IPC 抖动）。**证据（受控复验）**：daemon 日志显示 edit1 实际已启动（`Task [edit1] starting: sleep 20`）；STARTING→RUNNING 需等下一分钟 supervisor tick；独立复验 GET_SUMMARY 2-3s 响应、START_TASK 3/3 `rc=0 + state=RUNNING` → IPC 控制面健康。历史（09-03）此设备同用例多次 BLOCKED/FAIL。非产品缺陷 | 组合冒烟 daemon 忙窗口内 task start | 监控/放宽 item 14 的 IPC poll 窗口；或对 daemon 忙窗口做负载感知 | 否（待监控，不阻塞 P5 出口） |
