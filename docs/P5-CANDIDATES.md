...
| **O-1** | `tests/p1-device/smoke.sh` 5 连发 restart × service.sh watchdog | p1-device 5 连发 `su-scheduler restart` 与 watchdog（60s 轮询锁文件）竞态，crash-guard 偶发把优雅重启计为异常退出（crash_seq 0↔1 翻转），累计≥3 → 300s degraded 窗口 → 任务不执行 → smoke FAIL（P5-10 实测 4 轮中 3 轮受扰） | p1-device 组合跑 | 与 D-P5-05 同款：p1-device restart 前清 guard（测试基建）或专项排查 restart×watchdog 竞态 | 否（待处理，建议 P5-11 出口评审前处理） |
| **O-2** | daemon 主循环（su-schedulerd） | 主循环单次迭代偶发 >60s（实测 71s 间隙跳过整分钟 00:51），精确 HHMM 任务无 catch-up 补跑 | restart 风暴/降级窗口叠加 | 评估 catch-up 机制（P6 候选） | 否（P6 候选） |
| **O-3** | `su-scheduler task-config new` + `tcfg_new_id` | trigger 含空格（cron:0 8 * * *）时 `tcfg_new_id` 派生 ID 失败（明确报错无破坏）；cron 经 task 文件/`apply` 路径正常 | `task-config new` + cron trigger | 优化 `tcfg_new_id` 对含空格 trigger 的 ID 派生（后续任务） | 否 |
| **O-4** | EDIT_TASK / `su-scheduler ipc` | EDIT_TASK `configuration_invalid` 吞掉具体 stderr（daemon nohup 2>&1）；`ipc` 对已编码 payload 双编码被拒（行为正确但易误用） | 排查失败需走 task-config apply 复现 | 错误透传增强（后续任务） | 否 |
